...
Offline manifest inspection shows `app.getbb.mobile.MainActivity` exported with the `bb` URI scheme and no DevLauncher activity in the release manifest. Android's `Signature.hashCode` calculated from the candidate signing certificate is `[51ed3f60]`, exactly matching the `[51ed3f60]` signing marker previously observed for the installed `app.getbb.mobile` package. The candidate is therefore update-signature compatible with that observed installation, but it was not installed.
...
The physical iPhone was unavailable for WKS-1535's live behaviors. Xcode simulator infrastructure is healthy and a separate WKS-1534 simulator suite passed 12/12, but simulator proof cannot substitute for this issue's physical-device pairing, reconnect, revocation, and update checks.

| Behavior | Samsung | iPhone |
| --- | --- | --- |
| Pairing | Blocked: tablet unreachable | Blocked: physical iPhone unavailable |
| Reconnect | Blocked: tablet unreachable | Blocked: physical iPhone unavailable |
| Pending interaction | Blocked: tablet unreachable | Blocked: physical iPhone unavailable |
| Revocation | Blocked: tablet unreachable | Blocked: physical iPhone unavailable |
| Update and pairing-data survival | Blocked: no install performed | Blocked: physical iPhone unavailable |
